...
__all__ = ['HogFactory', 'DaisyFactory', 'flatten', 'LBPFactory', 'ConvolutionFeature']
...
class ConvolutionFeature(object):
    ...
    def __call__(self, arr: np.ndarray)->np.ndarray:
        out = np.dstack( [cv2.filter2D(arr, -1, self.kernel[:,:,k]) for k in range(self.kernel.shape[-1])] )
        return np.concatenate([ np.array(cv2.meanStdDev(out)).flatten() , (out>0).mean(axis=(0,1)) ], axis=0)