...
def lint(files):
    ...
    if not files:
        files = ("src/tests", "src/fashion_benchmark")
...
def build_docs(open_docs):
    ...
    python_call(
        "ipykernel", ["install", "--user", "--name=fashion_benchmark"]
    )
...
def _build_jupyter_command(
    ...
    if not all_kernels:
        project_name = "fashion_benchmark"
...
def convert_notebook(all_flag, overwrite_flag, filepath):
    ...
    *Note*: Make sure your notebooks have unique names!
    ...
    kedro_package_name = "fashion_benchmark"
