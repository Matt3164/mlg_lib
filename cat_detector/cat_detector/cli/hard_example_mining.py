...
from pypurr.common.helpers.functionnal.batch import batch
...
def _filter_w_metadata(data: List[Tuple[ndarray, ndarray]],
    ...
    keptX, keptMeta, _, _ = by_clf(X, list(map(lambda x: x[1], data)),
                                   ensemble.from_clfs(classifiers, threshold=THRESHOLD))
...
def _extract_negatives(run_id: int):
    ...
    data_iter = map(lambda x: resize_to_gray_flatten(x), data_iter)
